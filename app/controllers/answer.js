...
class AnswerController {
  ...
  async findById(ctx) {
    ...
    const Answer = await Answer.findById(ctx.params.id).select(selectFields).populate('questioner topics');
    ctx.body = Answer
  }
  ...
  async create(ctx) {
    ...
    const answer = await new Answer({ ...ctx.request.body, questionId,answerer }).save()
    ctx.body = answer;
  }
